...
other = read.csv('Perez/pass_model copy.csv')
...
#test <- left_join(pbp_data,other,by = c('game_date', 'period', 'clock_seconds', 'event', 'x_coord', 'y_coord'))

pbp_data <- pbp_data |> 
    left_join(select(other, game_date, period, clock_seconds, event, x_coord, y_coord, min_dist_from_pl_def_percent, sec_dist_from_pl_def_percent), 
