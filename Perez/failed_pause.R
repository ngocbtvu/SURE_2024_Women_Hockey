...
fin_swi_2_goal_p_2_clean <- fin_swi_2_goal_p_2_clean+
...
# Identifying pass frames
#pass_frames <- unique(final_fin_swi_2_goal_clean$frame_id[final_fin_swi_2_goal_clean$event == 'Play'])

# Adding pause frames manually
#frame_id_with_pause <- rep(final_fin_swi_2_goal_clean$frame_id, each = 2)
#frame_id_with_pause[seq(2, length(frame_id_with_pause), 2)] <- NA

#fin_swi_2_goal_p_2_clean <- fin_swi_2_goal_p_2_clean +
#  transition_manual(frame_id_with_pause)


#animate(can_usa_goal_p_2, renderer=av_renderer('Perez/animate_55.mp4'), duration = 10)

#-------------------------------------------------
animate(fin_swi_2_goal_p_2_clean, renderer=av_renderer(), duration = 12)

#------------------------------------------
#can_usa_goal_p_2 = plot_rink(ggplot(final_can_usa_goal)) +
#  geom_point(aes(x = x_ft, y = y_ft, fill = team_name.x), shape = 21, size = 6) +
#  geom_text(aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name.x), size = 3) +
...
#       subtitle = 'Pass Completion Probability : {closest_state} %',
#       fill = "Team") +
#  transition_states(difference_1, transition_length = 1, state_length = 1, wrap = FALSE)+
#  transition_time(frame_id) +
#  ease_aes()+
#  NULL
...
#-----------------------
#trying to add good legends
#fin_swi_2_goal_p_2_clean <- fin_swi_2_goal_p_2_clean + 
#  geom_point(x = fin_swi_2_points_clean$x_coord, y = fin_swi_2_points_clean$y_coord, 
#            data = fin_swi_2_points_clean, shape = 21, size = 6)+
#  scale_fill_manual(values = c('Shot' = 'green3', 'Takeaway' = 'pink', 'Puck Recovery' = 'hotpink')) +
#  geom_segment(x = final_fin_swi_2_goal_clean$x_coord, y = final_fin_swi_2_goal_clean$y_coord, 
#               xend = final_fin_swi_2_goal_clean$x_coord_2, yend = final_fin_swi_2_goal_clean$y_coord_2, colour = "skyblue",
#               linewidth = 1.2, arrow = arrow(length = unit(0.2, "inches")), data = final_fin_swi_2_goal_clean)+
#  geom_label(aes(label=str_wrap(final_fin_swi_2_goal_clean$x_coord,12), 
